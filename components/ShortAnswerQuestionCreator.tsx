        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 mb-4">
            ...
            <div className="mb-6">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                    Question <span className="text-xs text-slate-500">(LaTeX: e.g. $a^2 + b^2$)</span>
                </label>
                <textarea
                    value={question.question_text}
                    onChange={(e) => updateQuestion('question_text', e.target.value)}
                    placeholder="Enter a question that can be answered with a short phrase or sentence. Use $...$ or $$...$$ for math."
                        ...
                        <div key={index} className="flex gap-2">
                            <div className="flex-1">
                                <input
                                    ...
                                    placeholder="Acceptable answer (LaTeX supported)"
                                />
                            </div>
                            <button
                                onClick={() => removeCorrectAnswer(index)}
                                className="self-start px-3 py-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                            >
