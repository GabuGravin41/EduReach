...
export const UnitDetailPage: React.FC<UnitDetailPageProps> = ({ unitId, onBack, onSelectPaper, onPlayLesson }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-4 py-3 sm:py-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
            <div className="flex items-center gap-3">
              ...
              {hasCompetitionPapers && (
                <button
                  ...
                  className={`px-2 sm:px-2.5 py-1 text-xs font-semibold rounded-md transition-colors ${
                    competitionsOnly
                      ? 'bg-violet-600 text-white'
                      : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                  }`}
                >
                  🏆 <span className="hidden sm:inline">Competitions only</span><span className="sm:hidden">Comps</span>
                </button>
              )}
            </div>
            <div className="flex gap-1.5 sm:gap-2">
              <button
                ...
                className="px-2.5 sm:px-3.5 py-1.5 sm:py-2 border border-teal-500 text-teal-600 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-950/30 text-xs sm:text-sm font-semibold rounded-lg transition-colors"
              >
                Tag<span className="hidden sm:inline"> existing</span>
              </button>
              <button
                onClick={() => setAddPaperOpen(true)}
                className="px-2.5 sm:px-3.5 py-1.5 sm:py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs sm:text-sm font-semibold rounded-lg transition-colors"
              >
                + Add<span className="hidden sm:inline"> a past paper</span>
              </button>
            </div>
          </div>
          {papersLoading ? (
      ...
      {/* AI Tutor tab — taller on phones so the chat actually has room */}
      {activeTab === 'tutor' && (
        <div className="h-[calc(100vh-13rem)] sm:h-[70vh] min-h-[28rem]">
          <AIAssistant
            messages={messages}
            isLoading={aiLoading}
